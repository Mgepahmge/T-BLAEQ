...
CUDAKmeans::CUDAKmeans(const double* data, size_t N, size_t dim, bool is_aos) : N_(N), dim_(dim), k_(0) {
...
void CUDAKmeans::convertToPointVector(const double* flat_data, size_t N, size_t dim, bool is_aos) {
    datas_.clear();
    datas_.reserve(N);
...
void CUDAKmeans::uploadDataToGPU(const double* data, size_t N, size_t dim, bool is_aos) {
    dataset_gpu_ =
        std::make_unique<raft::device_matrix<double, int>>(raft::make_device_matrix<double, int>(*handle_, N, dim));

    if (is_aos) {
        cudaMemcpy(dataset_gpu_->data_handle(), data, N * dim * sizeof(double), cudaMemcpyHostToDevice);
    }
    else {
        ...
        cudaMemcpy(dataset_gpu_->data_handle(), temp.data(), N * dim * sizeof(double), cudaMemcpyHostToDevice);
    }
}
...
void CUDAKmeans::run(size_t k, size_t max_iters) {
    ...
    centroids_gpu_ =
        std::make_unique<raft::device_matrix<double, int>>(raft::make_device_matrix<double, int>(*handle_, k, dim_));
    labels_gpu_ = std::make_unique<raft::device_vector<int, int>>(raft::make_device_vector<int, int>(*handle_, N_));
    ...
    auto dataset_view = raft::make_device_matrix_view<const double, int>(dataset_gpu_->data_handle(), N_, dim_);

    double inertia = 0.0;
    int n_iter = 0;

    cuvs::cluster::kmeans::fit(*handle_, kmeans_params, dataset_view,
                               std::nullopt, // sample_weight
                               centroids_gpu_->view(), raft::make_host_scalar_view(&inertia),
                               raft::make_host_scalar_view(&n_iter));

    cuvs::cluster::kmeans::predict(*handle_, kmeans_params, dataset_view,
                                   std::nullopt, // sample_weight
                                   centroids_gpu_->view(), labels_gpu_->view(),
...
void CUDAKmeans::downloadResultsFromGPU() {
    ...
    cudaMemcpy(temp_labels.data(), labels_gpu_->data_handle(), N_ * sizeof(int), cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(temp_centroids.data(), centroids_gpu_->data_handle(), k_ * dim_ * sizeof(double),

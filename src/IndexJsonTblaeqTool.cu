...
namespace {
    ...
    void set_fixed_file_name(IndexRecordFixed& rec, const std::string& name, size_t file_name_bytes) {
        if (file_name_bytes > MAX_FILE_NAME_BYTES) throw std::runtime_error(
            "file_name_bytes exceeds MAX_FILE_NAME_BYTES");
        if (name.size() > file_name_bytes) throw std::runtime_error(
            "data file name exceeds fixed file_name_bytes: " + name);
    ...
    std::vector<Match> sort_limit_by_diff(std::vector<Match> m, size_t top_k) {
        std::sort(m.begin(), m.end(), [](const Match& a, const Match& b) {
            ...
            if (a.slot_ref.timestamp_us != b.slot_ref.timestamp_us) return a.slot_ref.timestamp_us < b.slot_ref.
                timestamp_us;
    ...
    std::vector<Match> query_interval(const std::vector<SlotRef>& slots, int64_t lo, int64_t hi,
                                      std::optional<int64_t> center, size_t top_k) {
    ...
    std::vector<Match> query_nearest(const std::vector<SlotRef>& slots, int64_t target, size_t top_k) {
        ...
        auto right = std::lower_bound(slots.begin(), slots.end(), target,
                                      [](const SlotRef& s, int64_t v) { return s.timestamp_us < v; });
        auto left = right;
    ...
    std::vector<uint8_t> read_payload_checked(const fs::path& output_dir, const IndexRecordFixed& rec, uint8_t slot) {
        ...
        if (in.gcount() != static_cast<std::streamsize>(payload.size())) throw std::runtime_error(
            "short payload read after length check");
        return payload;
    }

...
int main(int argc, char** argv) {
    ...
    auto* opt_seq = query->add_option("--seq", seq_filters,
                                      "sequence filter, accepts 1, seq0001, or a comma list");
    ...
    try {
        ...
        if (*query) {
            ...
            if (query_args.has_seq) query_args.seqs = parse_seq_list(seq_filters);
